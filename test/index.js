...
tape('simple', function (t) {
  ...
  t.deepEqual(acc, {data: [1], current: 0, width: 1000, max: 10})
  acc = reduce(acc, {ts: 1, value: 2})
  t.deepEqual(acc, {data: [3], current: 1, width: 1000, max: 10})
  acc = reduce(acc, {ts: 1001, value: 3})
  t.deepEqual(acc, {data: [3, 3], current: 1001, width: 1000, max: 10})
  acc = reduce(acc, {ts: 3001, value: 4})
  t.deepEqual(acc, {data: [3, 3, , 4], current: 3001, width: 1000, max: 10})
  console.log(acc)
  ...
  t.deepEqual(acc, {data: [3, 3, , 4, 5, 6, 7, 8, 9, 10], current: 9001, width: 1000, max: 10})
  ...
  t.deepEqual(acc, {data: [11, 3, , 4, 5, 6, 7, 8, 9, 10], current: 10001, width: 1000, max: 10})
  t.deepEqual(toArray(acc), [3, null, 4, 5, 6, 7, 8, 9, 10, 11])

  acc = reduce(acc, {ts: 12001, value: 12})
  t.deepEqual(acc, {data: [11, , 12, 4, 5, 6, 7, 8, 9, 10], current: 12001, width: 1000, max: 10})

  acc = reduce(acc, {ts: 15001, value: 15})
  t.deepEqual(acc, {data: [11, , 12, , , 15, 7, 8, 9, 10], current: 15001, width: 1000, max: 10})

  acc = reduce(acc, {ts: 21001, value: 22})
  t.equal(acc.data.length, 10)

  t.deepEqual(acc,{data: [, 22,12, , ,15,,,,,], current: 21001, width: 1000, max: 10})
  acc = reduce(acc, {ts: 26001, value: 26})
  t.deepEqual(acc,{data: [, 22,, , ,,26,,,,], current: 26001, width: 1000, max: 10})

  acc = reduce(acc, {ts: 64001, value: 64})
  t.deepEqual(acc,{data: [,,, ,64,,,,,,], current: 64001, width: 1000, max: 10})
